...
export const addBusiness = asyncHandler(async (req, res) => {
  ...
  try {
    let { username, phone, businessname, description, email, menuids } =
      req.body;
    ...
    if (user && user.role === "merchant") {
      throw new ApiError(400, "Merchant already exists with this phone number");
    }
    ...
    if (Array.isArray(menuids) && menuids.length > 0) {
      ...
      await AdminMenuPermissions.bulkCreate(menuPermissions, {
        transaction,
      });
    }
...
export const activeBusinessStatus = asyncHandler(async (req, res) => {
  try {
    const { bid, status, userid } = req.body;
    ...
    const existingUser = await AuthModel.findByPk(userid);
  ...
  try {
    ...
    if (businessinfoid) {
      ...
      if (req.file) {
        if (existingBusinessInfo.businessimage) {
          const oldPath = existingBusinessInfo.businessimage.startsWith("/")
            ? existingBusinessInfo.businessimage.substring(1)
      ...
      if (alreadyExists) {
        ...
        throw new ApiError(
          400,
          "Business info already exists for this business",
        );
      }
