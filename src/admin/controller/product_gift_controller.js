...
export const uploadUserGiftCard = asyncHandler(async (req, res) => {
  try {
    if (!req.file) {
      throw new ApiError(400, "All fields are required");
    }
...
export const getAllBusinessProducts = asyncHandler(async (req, res) => {
  try {
    const products = await ProductModel.findAll();
    return res.status(200).json(new ApiResponse(200, products));
  } catch (error) {
...
export const addGiftPack = asyncHandler(async (req, res) => {
  const { bid, giftpackname, description, giftpackprice } = req.body;

  try {
    if (!bid || !giftpackname?.trim() || !req.file) {
      throw new ApiError(400, "All fields are required");
    }

    const giftpackimage = `/${req.file.path.replace(/\\/g, "/")}`;

    const giftpack = await GiftPackModel.create({
      bid,
      giftpackname: giftpackname.trim(),
      giftpackimage,
      description,
      giftpackprice,
    });
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, "Gift pack added successfully"));
  } catch (error) {
...
export const getGiftPacks = asyncHandler(async (req, res) => {
  try {
    const { bid } = req.body;
    if (!bid) throw new ApiError(400, "Bid is required");

    const giftpacks = await GiftPackModel.findAll({
      where: { bid },
    });

    return res.status(200).json(new ApiResponse(200, giftpacks));
  } catch (error) {
    throw error;
  }
});

export const getAllGiftPacks = asyncHandler(async (req, res) => {
  try {
    const giftpacks = await GiftPackModel.findAll();
...
export const deleteGiftPack = asyncHandler(async (req, res) => {
  try {
    const { giftpackid, bid } = req.body;

    if (!giftpackid) throw new ApiError(400, "Gift pack id is required");
    if (!bid) throw new ApiError(400, "Bid is required");

    const giftpack = await GiftPackModel.findOne({
      where: { giftpackid, bid },
    });

    if (!giftpack) {
      throw new ApiError(404, "Gift pack not found");
    }

    const filepath = giftpack.giftpackimage.replace("/", "");
    if (fs.existsSync(filepath)) {
      fs.unlinkSync(filepath);
    }

    await giftpack.destroy();

    return res
      .status(200)
      .json(new ApiResponse(200, "Gift pack deleted successfully"));
  } catch (error) {
    throw error;
  }
});

export const createCustomGift = asyncHandler(async (req, res) => {
  const userid = req.user?.userid;
  ...
  for (const item of products) {
    ...
    if (!giftpackid) {
      throw new ApiError(400, "Gift Pack ID is required");
    }
  ...
  const createdItems = await CustomGiftItemModel.bulkCreate(customGiftItems);

  return res
    .status(200)
    .json(new ApiResponse(200, "Custom Gift Items added successfully"));
});

export const getCustomGifts = asyncHandler(async (req, res) => {
  try {
    const customgifts = await CustomGiftItemModel.findAll();

    res.status(200).json(new ApiResponse(200, customgifts));
  } catch (error) {
    throw error;
  }
});
