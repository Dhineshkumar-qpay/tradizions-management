...
export const addThinamoruKural = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await ThinamOruKuralModel.bulkCreate(kurals, {
      validate: true,
      ignoreDuplicates: true,
    });
...
export const updateKural = asyncHandler(async (req, res) => {
  try {
    const { kuralid, kural, meaning } = req.body;

    const existingKural = await ThinamOruKuralModel.findOne({
      where: {
        kuralid: kuralid,
      },
    });

    if (!existingKural) throw new ApiError(400, "Kural not found");
