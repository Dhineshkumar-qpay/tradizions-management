...
OrderModel.init(
  {
    ...
    discount_amount: {
      ...
      defaultValue: 0,
      get() {
        const rawValue = this.getDataValue("discount_amount");
        return rawValue ? parseFloat(Number(rawValue).toFixed(2)) : 0;
      },
    },
    ordertype: {
...
OrderItemModel.init(
  {
    ...
    itemtype: {
      type: DataTypes.ENUM("product", "gift", "customgift"),
      allowNull: false,
    },
    productid: {
      type: DataTypes.INTEGER,
...
OrderItemModel.belongsTo(GiftPackModel, {
  foreignKey: "giftpackid",
  as: "giftpack",
  constraints: false,
});
