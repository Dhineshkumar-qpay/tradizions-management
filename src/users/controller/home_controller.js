...
export const getAllProducts = asyncHandler(async (req, res) => {
  try {
    ...
    switch (pricerange) {
      case "0-500":
        priceCondition = { [Op.between]: [0, 500] };
        break;
    ...
    switch (sortby) {
      case "price-low-high":
        order = [
          [
            Sequelize.literal(
              `CASE WHEN sellingprice > 0 THEN sellingprice ELSE price END`,
            ),
            "ASC",
          ],
        ];
...
export const getProductDetail = asyncHandler(async (req, res) => {
  ...
  const relatedProducts = await ProductModel.findAll({
    where: {
      ...
      itemtype: "product",
      productid: { [Op.ne]: productDetail.productid },
    },
    attributes: [
      "productid",
      "bid",
...
export const giftDetails = asyncHandler(async (req, res) => {
  ...
  const relatedGifts = await ProductModel.findAll({
    where: {
      categoryid: gift.categoryid,
      itemtype: "gift",
      productid: { [Op.ne]: gift.productid },
    },
    attributes: [
      "productid",
      "bid",
